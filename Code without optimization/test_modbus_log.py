...
def test_modbus_log(target_testrig, logger, fsm_targets, unit, io_channel):
    ...
    response = modbus_check_read_invalid_coil(logger,fsm_targets,current_time)
    ...
    sleep(0.4)
    current_time = datetime.now()
    response = modbus_check_read_invalid_register(logger,fsm_targets,current_time)
    assert response <= 10.00
    ...
    current_time = datetime.now()
    sleep(5)
    response = modbus_check_read_invalid_coil_rate_limited(logger,fsm_targets,current_time)
    assert response <=10.00
    sleep(5)
    ...
    sleep(0.4)
    current_time = datetime.now()
    response = modbus_check_write_request(logger,fsm_targets,current_time)
    assert response <= 10.00
    sleep(5)
    ...
    current_time = datetime.now()
    sleep(5)
    response = modbus_check_write_request_ratelimiter(logger,fsm_targets,current_time)
    assert response <= 10.00
    sleep(5)
    ...
    sleep(0.4)
    current_time = datetime.now()
    response = modbus_check_read_valid_request(logger,fsm_targets,current_time)
    assert response <= 10.00
    sleep(5)
